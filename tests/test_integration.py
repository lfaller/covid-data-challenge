...
from covid_integration.data_loader import (
    load_owid_data, load_disease_sh_data, parse_disease_sh_json,
    compare_data_sources
)
from covid_integration.data_cleaner import (
    standardize_country_names, filter_valid_countries, 
    validate_data_quality, clean_owid_data, clean_api_data,
    identify_matching_countries, COUNTRY_NAME_MAPPING, EXCLUDE_REGIONS
)
from covid_integration.data_merger import (
    create_country_mapping_table, align_temporal_data, calculate_trend_metrics,
    merge_datasets, calculate_derived_metrics, generate_integration_summary,
...
class TestDataLoader:
    ...
    def test_parse_disease_sh_json(self):
        ...
        mock_api_data = [
            {
                'country': 'Afghanistan',
                'countryInfo': {'iso3': 'AFG', 'iso2': 'AF', '_id': 4, 'lat': 33, 'long': 65, 'flag': 'flag_url'},
                'population': 40000000,
                'cases': 234174,
                'deaths': 7896,
                'recovered': 180000,
...
class TestDataCleaner:
    ...
    def test_standardize_country_names_api(self):
        ...
        df = pd.DataFrame({
            'country': ['Bosnia', 'Cabo Verde', 'USA', 'Afghanistan']
        })
    ...
    def test_filter_valid_countries(self):
        ...
        df = pd.DataFrame({
            'country': ['Afghanistan', 'World', 'Europe', 'Albania', 'High income', 'Diamond Princess']
        })
...
class TestDataMerger:
    ...
    def test_create_country_mapping_table(self):
        ...
        assert afghanistan_row['in_owid'] == True
        assert afghanistan_row['in_api'] == True
        assert afghanistan_row['can_merge'] == True
        
        owid_only_row = mapping_table[mapping_table['country'] == 'OWID_Only'].iloc[0]
        assert owid_only_row['in_owid'] == True
        assert owid_only_row['in_api'] == False
        assert owid_only_row['can_merge'] == False
        
        api_only_row = mapping_table[mapping_table['country'] == 'API_Only'].iloc[0]
        assert api_only_row['in_owid'] == False
        assert api_only_row['in_api'] == True
        assert api_only_row['can_merge'] == False
    ...
    def test_align_temporal_data(self):
        ...
        afghanistan_row = owid_aligned[owid_aligned['country_standardized'] == 'Afghanistan'].iloc[0]
    ...
    def test_calculate_derived_metrics(self):
        ...
        expected_columns = [
            'cases_data_gap', 'cases_data_gap_percent',
            'deaths_data_gap', 'deaths_data_gap_percent',
            'current_cases_per_100k', 'current_deaths_per_100k',
...
class TestDataMergerIntegration:
    ...
    def test_full_integration_pipeline(self):
        ...
        owid_df = pd.DataFrame({
            ...
            'country_standardized': ['Afghanistan', 'Afghanistan', 'Albania', 'Albania', 'World'], 
        ...
        assert 'World' not in merged_df['country_standardized'].values if len(merged_df) > 0 else True
