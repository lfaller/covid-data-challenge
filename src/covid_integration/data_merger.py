...
def create_country_mapping_table(owid_df: pd.DataFrame, api_df: pd.DataFrame) -> pd.DataFrame:
    ...
    logger.info(f"Created country mapping table: {len(mapping_df)} total countries, "
                f"{mapping_df['can_merge'].sum()} can be merged")
    
    return mapping_df


def align_temporal_data(owid_df: pd.DataFrame, api_df: pd.DataFrame, 
                       alignment_strategy: str = 'latest') -> Tuple[pd.DataFrame, pd.DataFrame]:
    """
    ...
    if alignment_strategy == 'latest':
        ...
        latest_owid = owid_df.groupby('country_standardized').agg({
            'date': 'max',
            'total_cases': 'last',
            'total_deaths': 'last',
            'population': 'last',
        ...
        latest_owid.columns = [col if col in ['country_standardized', 'country', 'iso_code'] 
                              else f'owid_{col}' for col in latest_owid.columns]
        
        # API data represents current state, add prefix for clarity
        api_aligned = api_df.copy()
        api_aligned.columns = [col if col in ['country_standardized', 'country', 'iso_code'] 
                              else f'api_{col}' for col in api_aligned.columns]
        
        logger.info(f"Temporal alignment complete: OWID latest data from {latest_owid['owid_date'].max()}")
...
def calculate_trend_metrics(owid_df: pd.DataFrame, window_days: int = 30) -> pd.DataFrame:
    ...
    for country in recent_data['country_standardized'].unique():
        country_data = recent_data[recent_data['country_standardized'] == country].sort_values('date')
        
        if len(country_data) < 2:
            continue
            
        # Calculate trends
        first_cases = country_data['total_cases'].iloc[0] if pd.notna(country_data['total_cases'].iloc[0]) else 0
        last_cases = country_data['total_cases'].iloc[-1] if pd.notna(country_data['total_cases'].iloc[-1]) else 0
        
        first_deaths = country_data['total_deaths'].iloc[0] if pd.notna(country_data['total_deaths'].iloc[0]) else 0
        last_deaths = country_data['total_deaths'].iloc[-1] if pd.notna(country_data['total_deaths'].iloc[-1]) else 0
        
        # Calculate daily averages for new cases/deaths
        if 'new_cases' in country_data.columns:
            avg_daily_cases = country_data['new_cases'].mean() if not country_data['new_cases'].isna().all() else 0
        else:
            avg_daily_cases = 0
            
        if 'new_deaths' in country_data.columns:
            avg_daily_deaths = country_data['new_deaths'].mean() if not country_data['new_deaths'].isna().all() else 0
        else:
            avg_daily_deaths = 0
        
        # Calculate percentage change
        cases_pct_change = ((last_cases - first_cases) / first_cases * 100) if first_cases > 0 else 0
        deaths_pct_change = ((last_deaths - first_deaths) / first_deaths * 100) if first_deaths > 0 else 0
...
def merge_datasets(owid_df: pd.DataFrame, api_df: pd.DataFrame, 
    ...
    logger.info(f"Column dtypes for date columns: {merged_df[date_columns].dtypes.to_dict() if date_columns else 'None'}")
...
def calculate_derived_metrics(merged_df: pd.DataFrame) -> pd.DataFrame:
    ...
    if 'owid_population' in df.columns:
        if 'api_current_cases' in df.columns:
            df['current_cases_per_100k'] = (df['api_current_cases'] / df['owid_population'] * 100000).round(2)
        if 'api_current_deaths' in df.columns:
            df['current_deaths_per_100k'] = (df['api_current_deaths'] / df['owid_population'] * 100000).round(2)
...
def generate_integration_summary(merged_df: pd.DataFrame, 
                               country_mapping: pd.DataFrame = None) -> Dict:
    """
    ...
    key_columns = ['owid_total_cases', 'api_current_cases', 'owid_total_deaths', 'api_current_deaths']
    ...
    if 'owid_data_age_days' in merged_df.columns:
        ...
        summary['temporal_analysis']['avg_owid_data_age_days'] = float(merged_df['owid_data_age_days'].mean())
...
def integrate_covid_data(owid_df: pd.DataFrame, api_df: pd.DataFrame, 
                        output_dir: str = "outputs") -> Tuple[pd.DataFrame, Dict]:
    """
...
if __name__ == "__main__":
    ...
    try:
        ...
        print("\n=== Sample Merged Data ===")
        display_columns = ['country_standardized', 'owid_total_cases', 'api_current_cases', 
                          'cases_data_gap', 'current_cases_per_100k', 'avg_daily_new_cases']
        ...
        print(f"Average OWID data age: {summary['temporal_analysis'].get('avg_owid_data_age_days', 'N/A')} days")
        ...
        if summary['top_countries_by_cases']:
            ...
            for i, country in enumerate(summary['top_countries_by_cases'][:5], 1):
                print(f"{i}. {country['country_standardized']}: {country['api_current_cases']:,} cases")
