...
EXCLUDE_REGIONS = {
    ...
    'Lower middle income', 'Upper middle income', 'OECD countries',
    'International', 'MS Zaandam', 'Diamond Princess'
}
...
def standardize_country_names(df: pd.DataFrame, source: str = 'owid') -> pd.DataFrame:
    ...
    if source == 'owid':
        ...
        df_clean['country_standardized'] = df_clean['country'].map(COUNTRY_NAME_MAPPING).fillna(df_clean['country'])
        ...
        df_clean['country_standardized'] = df_clean['country'].map(reverse_mapping).fillna(df_clean['country'])
        logger.info(f"Applied reverse country name mappings for API data")
...
def filter_valid_countries(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df_filtered = df_filtered[~df_filtered['country'].str.contains(
        r'income|OECD|Union|International', case=False, na=False
    )]
    
    final_count = len(df_filtered['country'].unique())
    logger.info(f"Filtered countries: {initial_count} -> {final_count} "
...
def validate_data_quality(df: pd.DataFrame, source: str) -> Dict:
    ...
    case_death_cols = [col for col in df.columns if any(x in col.lower() 
                      for x in ['case', 'death', 'test']) and df[col].dtype in ['int64', 'float64']]
    ...
    logger.info(f"Quality assessment for {source}: {quality_metrics['total_countries']} countries, "
...
def clean_owid_data(df: pd.DataFrame) -> pd.DataFrame:
    ...
    cumulative_cols = ['total_cases', 'total_deaths', 'total_tests', 
    ...
    logger.info(f"OWID data cleaned: {len(df_clean)} rows, {df_clean['country'].nunique()} countries")
...
def clean_api_data(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if 'current_cases' in df_clean.columns and 'population' in df_clean.columns:
        df_clean['cases_per_100k'] = (df_clean['current_cases'] / df_clean['population'] * 100000).round(2)
    
    if 'current_deaths' in df_clean.columns and 'population' in df_clean.columns:
        df_clean['deaths_per_100k'] = (df_clean['current_deaths'] / df_clean['population'] * 100000).round(2)
    ...
    logger.info(f"API data cleaned: {len(df_clean)} rows, {df_clean['country'].nunique()} countries")
...
def identify_matching_countries(owid_df: pd.DataFrame, api_df: pd.DataFrame) -> Dict:
    ...
    logger.info(f"Country matching: {len(matched_countries)} matched, "
                f"{len(owid_only)} OWID-only, {len(api_only)} API-only")
    logger.info(f"Match rates: OWID {matching_stats['match_rate_owid']:.1f}%, "
                f"API {matching_stats['match_rate_api']:.1f}%")
    
    return matching_stats


def clean_all_data(owid_df: pd.DataFrame, api_df: pd.DataFrame) -> Tuple[pd.DataFrame, pd.DataFrame, Dict]:
    """
...
if __name__ == "__main__":
    ...
    try:
        ...
        print(f"OWID: {owid_raw['country'].nunique()} -> {owid_clean['country'].nunique()} countries")
        ...
        print(owid_clean[['country', 'country_standardized', 'date', 'total_cases', 'data_source']].head())
        
        print("\nAPI cleaned sample:")
        print(api_clean[['country', 'country_standardized', 'current_cases', 'cases_per_100k', 'data_source']].head())
